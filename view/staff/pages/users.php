...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Prepare data for the cURL request to send SMS
        $url = "http://localhost/vetsystem/pet_system_5%20(2)/pet_system_5/view/admin/pages/sms_acc_approval.php";
        $postData = [
            'id' => $id,
            'user_id' => $user_id,
            'phone_number' => $phone_number,
            'fullname' => $fullname,
            'status' => $status, // Pass the status (Approved/Declined)
            'submit' => true
        ];
        ...
        // Check the HTTP status code for cURL response
        if ($httpCode === 200) {
...
<head>
    ...
    <title>Cadiz City Veterinary Clinic</title>
